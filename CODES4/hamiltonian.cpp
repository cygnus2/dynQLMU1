...
void constH(){
   ...
   std::vector<int> rowscan(Wind.nflipMax+1);
   ...
   for(i=0;i<Wind.nBasis;i++){
      ...
      /* a single plaquette is arranged as 
        ...
        pwx  |   p   |  pyz
             ...
             o-------o
                pxy
      */
      for(j=0;j<=Wind.nflipMax;j++) rowscan[j]=NTOT;
      stateq = 0;
      for(p=0;p<VOL;p++){
     ...
     Wind.rows.push_back(curr_index);
     for(k=0;k<stateq;k++){
       Wind.cols.push_back(rowscan[k]+1);
       if(rowscan[k]==i) Wind.hamil.push_back(lam*n_Flip);
       else Wind.hamil.push_back(-1.0);
     }
   }
   ...
   //diag_LAPACK(Wind.hamil, Wind.evals, Wind.evecs);
   diag_LAPACK_RRR(Wind.hamil, Wind.evals, Wind.evecs);
}
