...
extern double schmidtDecom(std::vector<double>&, std::vector<std::vector<bool>>&, std::vector<std::vector<bool>>&,int, std::vector<MKL_INT>&);
...
void entanglementEntropy(int sector){
  ...
  std::cout<<"In routine entanglementEntropy "<<std::endl;
  ...
  for(p=0; p<sizet; p++){
    ...
    //printvec(sel_evec);
    EE = schmidtDecom(sel_evec,eA,eB,sector,sub2main);
  ...
  eA.clear(); eB.clear(); alpha.clear(); sub2main.clear(); 
}
...
double schmidtDecom(std::vector<double> &vec, std::vector<std::vector<bool>> &eA, 
    std::vector<std::vector<bool>> &eB,int sector, std::vector<MKL_INT> &sub2main){

  int i,j,p,k;
  int q; // to cross check results between scan(), binscan() and binscan2()
  int flagGI,count;
  double norm;
  ...
  for(j=0; j<DB; j++){
      ...
      // binscan() gives a wrong answer here. why? 
      p = Wind[sector].binscan2(conf);
      //q = Wind[sector].binscan(conf);
      //p = Wind[sector].scan(conf);
      //if(p!=q) std::cout<<"Mismatch between binscan2 and binscan, p="<<p<<" q="<<q<<std::endl;
