...
void entanglementEntropy(int sector){
  ...
  //std::cout<<"Subsystem A of length L= "<<LEN_A<<std::endl;
  //std::cout<<"Subsystem B of length L= "<<LEN_B<<std::endl;
  createBasis(sector);
  ...
  for(p=0; p<Wind[sector].trans_sectors; p++){
    ...
    for(i=0; i < Wind[sector].trans_sectors; i++){
       sel_evec.push_back(Wind[sector].evecs[p*Wind[sector].trans_sectors + i]);
    }
